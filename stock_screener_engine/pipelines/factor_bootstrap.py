...
from stock_screener_engine.data_sources.schemas import EquityValuationRecord, ShareholdingRecord
...
class FactorBootstrapPipeline:
    ...
    def ingest(
        ...
        coverage = {
            "financials": self.store.financial_statement_coverage(normalized_symbols, as_of=as_of, venue=canonical_venue),
            "valuations": self.store.equity_valuation_coverage(normalized_symbols, as_of=as_of, venue=canonical_venue),
        ...
        report = {
            ...
            "passed": all(value >= float(min_coverage) for value in coverage_values),
    ...
    def _ingest_valuations(
        ...
        for idx, row in enumerate(rows, start=2):
            ...
            try:
                ...
                rejected_rows += 1
                issues.append({"row": idx, "symbol": symbol or "", "severity": "error", "message": str(exc)})
